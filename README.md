...
- [ ] 상품이 프로모션을 가지고 있는지 반환한다.
- [ ] 상품의 이름이 비어있는지 검증한다.
- [ ] 상품의 가격이 0 보다 큰지 검증한다.
- [ ] 상픔의 수량이 0 보다 작은지 검증한다.

### 재고 관리 기능

- [ ] 상품 목록을 받아 재고를 생성한다.
- [ ] 하나의 상품에 두개 이상의 프로모션이 있는지 검증한다.
...
- [ ] 프로모션의 이름이 비어있는지 검증한다.
- [ ] 프로모션의 구매수량과 증정수량이 0보다 큰지 검증한다.
- [ ] 프로모션의 구매수량이 증정수량보다 작은지 검증한다.
- [ ] 프로모션의 시작날짜와 종료날짜가 비어있는지 검증한다.
- [ ] 프로모션의 시작날짜가 종료날짜 이전인지 검증한다.

### 프로모션 목록 관리 기능
- [ ] 프로모션 목록을 받아 생성한다.
- [ ] 상품의 프로모션 이름을 받아 해당 프로모션을 반환한다.
...
- [ ] 상품이 프로모션을 가지고 있는지 반환한다.
- [ ] 상품의 이름이 비어있다면 예외가 발생한다.
- [ ] 상품의 가격이 0 보다 크지 않다면 예외가 발생한다.
- [ ] 상픔의 수량이 0 보다 작다면 예외가 발생한다.

### 재고 관리 테스트

- [ ] 각 상품의 재고를 고려하여 주문 가능 여부를 확인한다.
- [ ] 하나의 상품에 두개 이상의 프로모션이 있다면 예외가 발생한다.
- [ ] 주문이 가능하지 않다면 예외가 발생한다.
- [ ] 주문이 이루어지면 해당 상품의 재고를 감소시킨다.

### 프로모션 테스트

- [ ] 프로모션의 이름, 구매수량, 증정수량, 시작날짜, 종료날짜를 입력받아 성공적으로 프로모션을 생성한다.
- [ ] 프로모션의 이름이 비어있다면 예외가 발생한다.
- [ ] 프로모션의 구매수량과 증정수량이 0보다 크지 않다면 예외가 발생한다.
- [ ] 프로모션의 구매수량이 증정수량보다 작다면 예외가 발생한다.
- [ ] 프로모션의 시작날짜와 종료날짜가 비어있다면 예외가 발생한다.
- [ ] 프로모션의 시작날짜가 종료날짜 이전이라면 예외가 발생한다.
- [ ] 프로모션 재고를 고려하여 적용 가능 여부를 확인한다.
- [ ] 구매 수량과 프로모션 적용 수량을 비교하여 구매 수량을 추가할지 확인한다.
- [ ] 구매 수량과 프로모션 적용 수량을 비교하여 프로모션 재고가 부족하면 일반 재고를 사용할지 확인한다.

### 프로모션 목록 관리 테스트
- [ ] 프로모션 목록을 받아 성공적으로 생성한다.
- [ ] 상품의 프로모션 이름을 받아 해당 프로모션을 반환한다.
- [ ] 상품의 프로모션 이름에 해당되는것이 없다면 null을 반환한다.
