...
### 사용자 입력 검증 및 변환 기능 (InputValidatingParser)
...
- [ ] 상품 파일을 읽어 상품 목록을 반환한다.
- [ ] 프로모션 파일을 읽어 프로모션 목록을 반환한다.

### 상품 파일 내용 검증 및 변환 기능

- [ ] 상품의 첫번째 줄을 ','(콤마)로 나눈 길이와 열의 길이가 같은지 검증한다.
- [ ] 상품을 ','(콤마)로 나눈 길이와 열의 길이가 같은지 검증한다.
- [ ] 상품의 가격이 양의 숫자인지 검증한다.
- [ ] 상품의 가격이 최대 범위를 넘어서는지 검증한다.
- [ ] 상품의 가격이 0보다 큰지 검증한다.
- [ ] 상품의 수량이 양의 숫자인지 검증한다.
- [ ] 상품의 수량이 최대 범위를 넘어서는지 검증한다.
- [ ] 상품의 수량이 0과 같거나 큰지 검증한다.

### 프로모션 파일 내용 검증 및 변환 기능

- [ ] 프로모션의 첫번째 줄을 ','(콤마)로 나눈 길이와 열의 길이가 같은지 검증한다.
- [ ] 프로모션을 ','(콤마)로 나눈 길이와 열의 길이가 같은지 검증한다.
- [ ] 프로모션 제공 기준 수량이 양의 숫자인지 검증한다.
- [ ] 프로모션 제공 기준 수량이 최대 범위를 넘어서는지 검증한다
- [ ] 프로모션 제공 수량이 양의 숫자인지 검증한다.
- [ ] 프로모션 제공 수량이 최대 범위를 넘어서는지 검증한다
- [ ] 프로모션 시작 날짜와 종료 날짜의 형식이 유효한지 검증한다.
- [ ] 프로모션 시작 날짜가 종료 날짜 이전인지 검증한다.
...
### 사용자 입력 검증 및 변환 테스트 (InputValidatingParser)
...
- [ ] 상품 파일을 읽어 원시 상품 목록을 반환한다.
- [ ] 상품 파일이 없다면 예외가 발생한다.
- [ ] 빈 상품 파일이라면 예외가 발생한다.
- [ ] 프로모션 파일을 읽어 원시 프로모션 목록을 반환한다.
- [ ] 프로모션 파일이 없다면 예외가 발생한다.
- [ ] 빈 프로모션 파일이라면 예외가 발생한다.

### 상품 파일 내용 검증 및 변환 테스트

- [ ] 상품 파일 내용을 읽어 원시 상품 목록을 반환한다.
- [ ] 상품의 첫번째 줄을 ','(콤마)로 나눈 길이와 열의 길이가 같지 않으면 예외가 발생한다.
- [ ] 상품을 ','(콤마)로 나눈 길이와 열의 길이가 같지 않으면 예외가 발생한다.
- [ ] 상품의 가격이 숫자가 아니면 예외가 발생한다.
- [ ] 상품의 가격이 최대 범위를 넘어서면 예외가 발생한다.
- [ ] 상품의 가격이 0과 같거나 작으면 예외가 발생한다.
- [ ] 상품의 수량이 숫자가 아니면 예외가 발생한다.
- [ ] 상품의 수량이 최대 범위를 넘어서면 예외가 발생한다.
- [ ] 상품의 수량이 0보다 작으면 예외가 발생한다.

### 프로모션 파일 내용 검증 및 변환 테스트

- [ ] 프로모션 파일 내용을 읽어 원시 프로모션 목록을 반환한다.
- [ ] 프로모션의 첫번째 줄을 ','(콤마)로 나눈 길이와 열의 길이가 같지 않으면 예외가 발생한다.
- [ ] 프로모션을 ','(콤마)로 나눈 길이와 열의 길이가 같지 않으면 예외가 발생한다.
- [ ] 프로모션 제공 기준 수량이 숫자가 아니면 예외가 발생한다.
- [ ] 프로모션 제공 기준 수량이 최대 범위를 넘어서면 예외가 발생한다.
- [ ] 프로모션 제공 수량이 숫자가 아니면 예외가 발생한다.
- [ ] 프로모션 제공 수량이 최대 범위를 넘어서면 예외가 발생한다.
- [ ] 프로모션 시작 날짜의 형식이 유효하지 않으면 예외가 발생한다.
- [ ] 프로모션 종료 날짜의 형식이 유효하지 않으면 예외가 발생한다.
- [ ] 프로모션 시작 날짜가 종료 날짜가 이후이면 예외가 발생한다.
